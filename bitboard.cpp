...
BitBoard::BitBoard(bool newgame){
	nummoves = 0;
	score = 0;
	outcome = -1;
	sides[0] = 0;
	...
	scorefunc = &ScoreBitBoard::getscore;
}

BitBoard::BitBoard(uint64_t hash){
	nummoves = 0;
	score = 0;
	outcome = -1;
	sides[0] = 0;
...
BitBoard::BitBoard(const char * str){
	nummoves = 0;
	score = 0;
	outcome = -1;
...
char BitBoard::won_calc() const {
	char wonside = 0;

	for(int i = 0; i < 32; i++){
		if(     (sides[0] & winmaps[i]) == winmaps[i]) wonside |= 1; 
		else if((sides[1] & winmaps[i]) == winmaps[i]) wonside |= 2;
	}
...
/*
int BitBoard::getchildren(BitBoard * children, bool dohash, bool doscore){
	uint64_t hashes[(36 - nummoves)*8];
	uint64_t * hashend = hashes;
	uint64_t hash;

	BitBoard * newboard = children;
...
void BitBoard::move(int pos, int spin){
	score = 0;
...
void BitBoard::spinquadrant(int spin){
	switch(spin){
...
inline void BitBoard::spinpartcw(int x, int y){
	int temp;
